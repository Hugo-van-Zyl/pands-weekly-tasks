...
#I am assuming a valid account number is anything with at least 4 digits
...
while True: #use while loop to iterate through input until the user has inputted a valid account number
        ...
        account_nr_length = len(account_nr) #use function to get the lenght of the input string

        #For this task I assume any account number with at least 4 digit is valid and that a valid account number can only consist of digits i.e. no decimals, characters, spaces etc
        if (account_nr_length>=4 and account_nr.isdigit()==True): #if these 2 conditions are met, run the below code. Use the isdigit() function to check if all numbers were entered
            ...
            print("Error - your input was invalid - only enter numbers between 0 and 9 (no decimals, spaces or symbols)") #display error message to the user


print("The last 4 digits of your account number is - ","X"*(account_nr_length-4),account_nr[-4:], sep="") 
